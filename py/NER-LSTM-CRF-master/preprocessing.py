...
def build_vocabulary(path_data, path_vocs_dict, min_counts_dict, columns,
    ...
    构建字典
    Args:
        path_data: str, 训练数据 数据路径
        path_vocs_dict: dict, 字典存放路径
        min_counts_dict: dict, item最少出现次数
        columns: list of str, 每一列的名称  f1
        sequence_len_pt: int，句子长度百分位
        use_char_featrue: bool，是否使用字符特征(针对英文)  英文的可以改成true
    ...
    for i in range(len(columns)):
        feature_item_dict_list.append(defaultdict(int)) #列表中存字典
    # char feature
    if use_char_featrue:
        char_dict = defaultdict(int)
        word_length_list = []  # 单词长度
    sequence_length = 0  #句子长度 ？
    ...
    while line:
        ...
        items = line.split('\t')
        sequence_length += 1
        # print(items)  #列表中  feature_item_dict_list 存的是dict ，意思是，每个dict中 item 计数
...
def main():
    ...
    # 加载配置文件
    with open('./config.yml','r',encoding= 'utf-8') as file_config:
